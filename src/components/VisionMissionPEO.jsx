import { Eye, Target, GraduationCap, ArrowRight } from "lucide-react";
...
function VisionMissionPEO({ showCTA = false }) {
  return (
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 border-l-4 border-l-brand-blue p-6 sm:p-7 flex flex-col">
            <div className="max-w-[95%]">
              ...
              <p className="text-gray-600 text-sm leading-relaxed text-justify [text-justify:inter-word]">
                To be globally recognized through excellence in Outcome Based
                Education utilizing the potential of Application Oriented
                Research and Innovation in the field of Computer Applications.
                The holistic approach rendered in amalgamating the academic
                progress of the students with the niche of cutting edge
                technology rightly proclaims to bridge the industry-academia
                gap. The departmental galaxy of the finest students in tune with
                the erudite teaching fraternity has always remained committed in
                bringing out the best in terms of academic accomplishments and
                creative excellence.
              </p>
            </div>
          </div>
              ...
              <ul className="space-y-3">
                {mission.map((m) => (
                  <li key={m.id} className="flex gap-2.5">
                    <span className="text-xs font-semibold text-orange-500 shrink-0 mt-0.5">
                      {m.id}
                    </span>
        ...
        {showCTA && (
          <div className="mt-8 flex justify-center">
            <Link
              to="/vision-mission"
              className="inline-flex items-center gap-2 rounded-full border border-[#081F5C]/15 bg-white px-5 py-3 text-sm font-medium text-[#081F5C] shadow-sm transition-all duration-300 hover:border-[#081F5C] hover:bg-[#081F5C] hover:text-white hover:shadow-md"
            >
        ...
        {/* Full PEO Section (Only on dedicated page) */}
        {!showCTA && (
              ...
              {peos.map((peo) => (
                <div key={peo.id} className="flex gap-3">
                  <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                    <GraduationCap
                      className="w-[18px] h-[18px] text-brand-blue"
                      strokeWidth={2}
                    />
                  </div>
